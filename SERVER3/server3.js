...
server2.on("delete-database-order",(data)=>{
  eventEmitter.emit("delete-database-order-e",data);
})

server1.on("delete-database-order",(data)=>{
  eventEmitter.emit("delete-database-order-e",data);
})

server4.on("delete-database-order",(data)=>{
  eventEmitter.emit("delete-database-order-e",data);
})

server2.on("admin-order-status-update",(data)=>{
  ...
  Order.deleteOne({_id:orderId},(err,result) => {
    if(err){
      ...
      for(var i = 1;i<=totalServers;i++)
      {
        if(i!=selfServerId)
        {
          io.to(i).emit("delete-database-order",orderId);
        }
      }
